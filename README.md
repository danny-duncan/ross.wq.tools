...
flowchart LR
	subgraph 2. Preprocessing & Tidying
        Tracker_File[(HV_Tracking.parquet)] -->|munge_api_data| Standardized_Raw[Raw Sensor Dataframe]
        ...
        Notes_Visit[Field Site Visit Notes] -->|add_field_notes| Join_Visit[Metadata-Joined Dataset]
...
flowchart LR
    subgraph 3. Tier 1 QAQC Flagging [Single-Sensor]
        Stats_Df[Statistical Bounds & Seasons Assigned] -->|add_field_flag| F1[Field Crew Visited Flagged]
...
flowchart LR
    subgraph 4. Tier 2 & 3 QAQC Flagging [Inter-Sensor & Spatial Network]
        F8[Seasonal Out-of-Bound Flagged] -->|add_frozen_flag| I1[Frozen Water Conditions Flagged]
...
```
### 5. Back-Calibration & Drift Correction
```mermaid
flowchart LR
    subgraph 5. Back-Calibration & Drift Correction
        QC_Complete[Final QAQC Flagged Dataset] -->|cal_join_sensor_calibration_data| Joined_Cal[Sensor + Calibrations Linked]
        Cal_Collate[(munged_calibration_data.RDS)] --> Joined_Cal
...
```
### 6. Data Compile & Output

```mermaid
flowchart LR
    %% Final Stage
    subgraph 6. Data Compile & Output
        Final_Out[Linear/Exponential Correction] -->|final_data_binder / select columns| Output_Compiled[Clean Final Dataset]
